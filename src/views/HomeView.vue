<template>
    <div>
        ...
        <input type="search" class="search" name="eirbsearch" ref="searchInput" placeholder="Rechercher..."
            autocomplete="false" autofocus v-model="search" v-on:keyup.enter="tada()" />
        ...
        <div class="container">
            <div id="overall" class="home">
                ...
                <TransitionGroup tag="div" :css="false" @before-enter="onBeforeEnterFn" @enter="onEnterFn"
                    @leave="onLeaveFn">
                    <section v-for="linkGroup in filteredLinkGroups" :id="linkGroup.id" :key="linkGroup.id">
                        <h3>{{ linkGroup.name }}</h3>

                        <TransitionGroup tag="div" :css="false" @before-enter="onBeforeEnterFn" @enter="onEnterFn"
                            @leave="onLeaveFn" class="cards">
...
onMounted(() => {
    searchInput.value = document.getElementById(
        "search"
    ) as HTMLInputElement;
